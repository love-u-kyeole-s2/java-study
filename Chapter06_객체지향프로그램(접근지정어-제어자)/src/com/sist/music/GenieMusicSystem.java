 ...
 *   	----
 *   		jdk17
 *   	jakarata==>apache(오픈소스 그룹)
 *   2.패키지에 있는 클래스 가지고 오기
 *   	=>다른 패키지에 있는 클래스는 인식을 못한다.
 *   	=>import 패키지.* :패키지 안에 있는 모든 클래스
 *   	  import 패키지.클래스명: 지정된 클래스 1개
 *   3.접근지정어
 *   -----------------------------------------------------------
 *   private: 데이터 은닉화
 ...
 *   -----------------------------------------------------------
 *   1) 인스턴스 변수: private(외에 4개 사용이 가능이만 대부분은 다 private씀)
 *   		=>데이터 노출(X)
 *   		=>웹: 노출
 *   			POST login.jsp
 *   			GET=> login.jsp?id=hong&pwd=1234
 *   2)클래스 :default,public
 *   3) 메소드: 가급적이면 public/다른 클래스와 연결
 *   4) 생성자:public/다른 클래스 연결
 *   4.제어어
 *   	static:공통의
 *   	final:마지막인
 *   	abstract:추상적인...=>메소드가 선언만
 *   	--------
 *   	 윈도우:버튼/텍스트
 *   	 추상클래스/***인터페이스(스프링)
 *   	
...
public class GenieMusicSystem {
	...
	//초기화를 시킨다면=>스태틱이니까 객체생성없이 초기화=>스태틱블록을쓴다 
	//시작하자마자 메소드를 호출하거나 이런거없이 바로실행시키려고하고있음
	//1번만읽기때문에 메모리공간이런게 편하다
	static//자동호출되는 프로그램=>스태틱은 상속 예외임
	{//스태틱안에는 인스턴스 못들어감
		try
		{
