...
public class DataCollection {
	...
	public static void main(String[] args) {
		...
		try
        {
//        	FoodDAO dao=FoodDAO.newInstance(); <=데이터베이스와 관련된거
			int m=1;
        	for(int i=1;i<=346;i++)
        	{
        		Document doc=
        		...
        		for(int j=0;j<link.size();j++)
        		{
        		  try
        		  {
        		    ...
        		    */
//        		   dao.foodInsert(vo);
        		   String data=m+"|"+vo.getName()+"|"+vo.getType()+"|"+vo.getPhone()+"|"+vo.getAddress()+"|"+vo.getScore()+"|"+vo.getParking()+"|"+vo.getPoster()+"|"+vo.getTime()+"|"+vo.getContent()+"|"+vo.getTheme()+"|"+vo.getPrice()+"\r\n";
        				   m++;
        		   fw.write(data);
        		   fw.close();
        		  }catch(Exception ex){}
