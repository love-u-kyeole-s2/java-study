 ...
 * 		long(8byte)	=> 빅데이터
 * 		실수
 * 		double: 8byte==> 숫자중에 가장 큰 데이터형
 * 				소수점이 필요시에 사용 =>정밀도 소수점 이하 15자리
 * 		논리
 ...
 * 		 | 데이터형+ 클래스형
 * 		*** 웹/네트워크는 모든 데이터가 문자열이다 이거 꼭기억하셈
 * 			-----------------------------------------변환
 * 			valueOf()
 * 		=> 웹 : String, double,int
 ...
 * 				  double
 * 				2)int 이하 연산자
 * 					byte, char, short=>연산결과는 int
 * 				3) 자동 형변환 : 작은 데이터형 ==> 큰데이터형으로 전송
 * 				   강제 형변환: 큰데이텨형 =>작은 데이터형으로 변경
 * 				  --------------------->자동
 ...
 * 					조건 false
 * 				}
 * 				=========>웹에서제일많이씀 로그인 여부/ 관리자냐 사용자냐
 * 				=========>if else 많은이유 일반프로그램은 안맞으면건너뛰면되는데
 * 						  웹이란 프로그램은 무조건 값을 보내야함 실패했던안했던
 * 						  그래야 화면을 유지함 
 * 						  HTML은 UI=>기능 첨부(자바)
 * 						  --------------------------JSP=>ThymeLeaf
